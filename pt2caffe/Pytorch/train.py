...
def train_classification_net(net,
                             trainloader,
                             testloader=None,
                             save_path='/tmp/pytorch_train_tmp.pth',
                             base_lr=0.01,
                             num_epoch=5,
                             use_cuda=True,
                             optimizer=None,
  ...
  for epoch in range(num_epoch):
    ...
    for iter_idx, (inputs, labels) in enumerate(trainloader, 0):
      ...
      if iter_idx % print_iter == 0:
        logger(
            "Epoch-Iter [%d/%d][%d/%d] Time_tot/load [%f][%f] loss [%f] Prec@1 [%f]"
            % (epoch + 1, num_epoch, iter_idx, len(trainloader), batch_time.avg,
               load_time.avg, running_loss.avg, top1.avg))
        running_loss.reset()
        load_time.reset()
        batch_time.reset()
        top1.reset()

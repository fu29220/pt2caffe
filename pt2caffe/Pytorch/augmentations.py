...
class RandomSaturation(object):
  ...
  def __call__(self, image, *args):
    if random.randint(2):
      image[:, :, 1] = (image[:, :, 1] *
                        random.uniform(self.lower, self.upper)).astype(np.uint8)
    if len(args):
      return (image, *args)
    else:
      return image
...
class RandomLightingNoise(object):
  ...
  def __init__(self):
    self.perms = ((0, 1, 2), (0, 2, 1), (1, 0, 2), (1, 2, 0), (2, 0, 1), (2, 1,
                                                                          0))
  ...
  def __call__(self, image, *args):
    ...
    if self.type == 'constant':
      image = cv2.copyMakeBorder(
          ...
          size,
          cv2.BORDER_CONSTANT,
          value=self.constant_color)
    elif self.type == 'reflect':
      image = cv2.copyMakeBorder(image, size, size, size, size,
                                 cv2.BORDER_REFLECT)

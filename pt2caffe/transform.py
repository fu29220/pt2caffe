...
# support the inplace output of the layers
class Blob_LOG():
...
def _conv2d(raw,
  ...
  if groups == input.shape[1] == x.shape[1]:
    name = log.add_layer(name='dwconv')
    log.add_blobs([x], name='dwconv_blob')
  ...
  layer = caffe_net.Layer_param(
      name=name, type=conv_type, bottom=[log.blobs(input)], top=[log.blobs(x)])
  layer.conv_param(
      x.size()[1],
      weight.size()[2:],
      stride=_pair(stride),
...
def _split(raw, tensor, split_size, dim=0):
  ...
  layer = caffe_net.Layer_param(
      name=layer_name, type='Slice', bottom=[log.blobs(tensor)], top=top_blobs)
...
def _pool(type, raw, input, x, kernel_size, stride, padding, ceil_mode):
  # dilation,ceil_mode,return indices
  layer_name = log.add_layer(name='{}_pool'.format(type))
  top_blobs = log.add_blobs([x], name='{}_pool_blob'.format(type))
  layer = caffe_net.Layer_param(
      name=layer_name, type='Pooling', bottom=[log.blobs(input)], top=top_blobs)
  # w,h different kernel, stride and padding
...
def _adaptive_avg_pool2d(raw, input, output_size):
  ...
  print("***********adaptive_avg_pool", output_size, input.shape[2])
  x = raw(input, output_size)
  _pool('ave', raw, input, x, input.shape[2] - output_size + 1, 1, 0, False)
  return x
...
def _max(raw, *args):
  ...
  if len(args) == 1:
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type='Eltwise', bottom=bottom_blobs, top=top_blobs)
...
def _cat(raw, inputs, dimension=0):
  ...
  layer = caffe_net.Layer_param(
      name=layer_name, type='Concat', bottom=bottom_blobs, top=top_blobs)
...
def _dropout(raw, input, p=0.5, training=False, inplace=False):
  ...
  layer = caffe_net.Layer_param(
      name=layer_name, type='Dropout', bottom=bottom_blobs, top=top_blobs)
...
def _threshold(raw, input, threshold, value, inplace=False):
  ...
  if threshold == 0 and value == 0:
    ...
    layer = caffe_net.Layer_param(
        name=name, type='ReLU', bottom=bottom_blobs, top=[log.blobs(x)])
    log.cnet.add_layer(layer)
    return x
  if value != 0:
  ...
  layer = caffe_net.Layer_param(
      name=layer_name, type='Threshold', bottom=bottom_blobs, top=top_blobs)
...
def _relu(raw, input, inplace=False):
  ...
  log.add_blobs([x], name='relu_blob')
  layer = caffe_net.Layer_param(
      name=name, type='ReLU', bottom=[log.blobs(input)], top=[log.blobs(x)])
  log.cnet.add_layer(layer)
  return x
...
def _prelu(raw, input, weight):
  ...
  layer = caffe_net.Layer_param(
      name=name, type='PReLU', bottom=bottom_blobs, top=[log.blobs(x)])
  if weight.size()[0] == 1:
...
def _tanh(raw, input):
  ...
  layer = caffe_net.Layer_param(
      name=name, type='TanH', bottom=[log.blobs(input)], top=[log.blobs(x)])
  log.cnet.add_layer(layer)
  return x
...
def _softmax(raw, input, dim=None, _stacklevel=3):
  ...
  layer = caffe_net.Layer_param(
      name=name, type='Softmax', bottom=bottom_blobs, top=[log.blobs(x)])
...
def _batch_norm(raw,
  ...
  layer1 = caffe_net.Layer_param(
      name=layer_name1, type='BatchNorm', bottom=bottom_blobs, top=top_blobs)
  ...
  if weight is not None and bias is not None:
    ...
    layer2 = caffe_net.Layer_param(
        name=layer_name2, type='Scale', bottom=top_blobs, top=top_blobs)
...
def _instance_norm(raw,
                   ...
                   momentum=0.1,
                   eps=1e-5):
  # the batch size!=1 view operations
  print(
  ...
  if running_var is not None or weight is not None:
    # the affine=True or track_running_stats=True case
...
def _interpolate(raw,
  ...
  layer.upsample_param(
      size=(input.size(2), input.size(3)), scale_factor=scale_factor)
  log.cnet.add_layer(layer)
  return x
...
def _sigmoid(raw, input):
  ...
  layer = caffe_net.Layer_param(
      name=name, type='Sigmoid', bottom=[log.blobs(input)], top=[log.blobs(x)])
  log.cnet.add_layer(layer)
  return x
...
def _hardtanh(raw, input, min_val, max_val, inplace):
  ...
  layer = caffe_net.Layer_param(
      name=name, type='ReLU6', bottom=[log.blobs(input)], top=[log.blobs(x)])
  log.cnet.add_layer(layer)
  return x
...
def _view(input, *args):
  ...
  layer = caffe_net.Layer_param(
      name=layer_name, type='Reshape', bottom=[log.blobs(input)], top=top_blobs)
  # reshpae added to nn_tools layer
  dims = list(args)
...
def _mul(input, *args):
  ...
  if not NET_INITTED:
    return x
  print("******************mul", input.shape, args[0].shape)
  if not input.shape == args[0].shape and input.shape[:2] == args[
      0].shape[:2] and list(args[0].shape[2:4]) == [1, 1]:
    y = torch.flatten(args[0], 1)
    #layer_name = log.add_layer(name='reshape')
    #top_blobs = log.add_blobs([y], name='reshape_blob')
    #layer = caffe_net.Layer_param(
    #    name=layer_name,
    #    type='Reshape',
    #    bottom=[log.blobs(args[0])],
    #    top=top_blobs)
    #dims = list(args[0].shape[:2])
    #print("-----------------------reshpae", dims)
    #layer.param.reshape_param.shape.CopyFrom(caffe_net.pb.BlobShape(dim=dims))
    #log.cnet.add_layer(layer)

    layer_name = log.add_layer(name='scale')
    top_blobs = log.add_blobs([x], name='scale_blob')
    layer = caffe_net.Layer_param(
        name=layer_name,
        type='Scale',
        bottom=[log.blobs(input), log.blobs(y)],
        top=top_blobs)
...
def _permute(input, *args):
  ...
  layer = caffe_net.Layer_param(
      name=name, type='Permute', bottom=[log.blobs(input)], top=[log.blobs(x)])
  order1 = args[0]
  order2 = args[1]
  order3 = args[2]
  order4 = args[3]
...
torch.flatten = Rp(torch.flatten, _reshape)
torch.sigmoid = Rp(torch.sigmoid, _sigmoid)
torch.tanh = Rp(torch.tanh, _tanh)
#torch.mul=Rp(torch.mul,_mull)

# other types of the view function
try:

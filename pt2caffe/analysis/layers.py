  ...
  def __init__(self, input, name=''):
    ...
    def transfer_input(_input):
      if isinstance(_input, Base):
        ...
        assert isinstance(
            input, Blob
        ), 'The input of layer %s is not Blob, please use nn_tools.P.blob.Blob as input' % name
      return _input
  ...
  def __getattribute__(self, item):
    if item == 'ops':
      try:
        ...
        print("CRITICAL WARNING: Layer {} ops cannot be calculated, set to 0."
              .format(self.name))
        self.ops = 0
...
class Sliding(Base):
  ...
  def __init__(self,
    ...
    if self.input.dim == 4:
      ...
      raise ValueError(
          'Sliding must have a input with 2D Map(batch,c,w,h) or 3D Map(batch,c,d,w,h)'
      )
    ...
    self.layer_info = 'kernel=%s,stride=%s,pad=%s' % ('x'.join([
        str(_) for _ in self.kernel_size
    ]), 'x'.join([str(_) for _ in self.stride]), 'x'.join(
        [str(_) for _ in self.pad]))
    if transpose:
    ...
    for i in range(self.input.dim - 2):
      if not transpose:
        if not ceil:
          outs.append(
              np.floor(
                  float(self.input[2 + i] + self.pad[i] * 2 -
        ...
        # transpose
        outs.append((self.input[2 + i] - 1) * self.stride[i] - 2 * self.pad[i] +
                    self.kernel_size[i])
    #     if not ceil:
...
class Conv(Sliding):
  ...
  def __init__(self,
    ...
    self.dot = np.prod(self.out.shape) * np.prod(
        self.kernel_size) * self.in_channel
...
class Pool(Sliding):
  ...
  def __init__(self,
    ...
    if isinstance(input, Base):
      input = input()
    Sliding.__init__(
        self, input, kernel_size, input.c, stride, pad, name=name, ceil=ceil)
    ...
    if pool_type in ['max', 0]:
      ...
      print("WARNING, NOT IMPLEMENT POOL TYPE %s PROFILING at %s, CONTINUE" %
            (pool_type, name))
